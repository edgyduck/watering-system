...
int moistureLowLimit = 20; // Set low limit (percent) for moisture and water sensors
int waterLowLimit = 5;
int checkHour = 17;   // Run moisture check at 17:15 each day
int checkMinute = 15;
...
void loop() {
 ...
 if(currDay - lastCheckDay != 0 && strcmp(time, "17:30") == 0) // if it's not the same day and time is 17:15 then do the job
...
int getWaterLevel(int sensorPin)
{
  ...
  lcd.setCursor(0,1);
  lcd.print("Vandens: ");
  lcd.print(waterLvlPercent);
  lcd.print("%      ");
  delay(1000);
  return waterLvlPercent;
}
